...
class MCTSDataAnalyzer:
    ...
    def analyze_bidding_patterns(self):
        ...
        for item in hand_strength_bid_correlation:
            strength = item['hand_strength']
            if strength < 0.2:
                strength_ranges['weak (0.0-0.2)'].append(item)
            elif strength < 0.4:
                strength_ranges['medium (0.2-0.4)'].append(item)
            elif strength < 0.6:
                strength_ranges['strong (0.4-0.6)'].append(item)
            else:
                strength_ranges['very_strong (0.6+)'].append(item)
        
        for range_name, items in strength_ranges.items():
            if items:
    ...
    def generate_training_data(self) -> List[Dict[str, Any]]:
        """Generate training data from MCTS games for RL model"""
        training_data = []
