...
class PointPredictionDataset(Dataset):
    ...
    def load_mcts_data(self, mcts_data_file: str):
        ...
        try:
            ...
            for example in mcts_data['training_data']:
                ...
                # Skip incomplete games (28-0 games)
                if example.get('game_complete', True) == False:
                    continue
                    
                if len(hand) >= 4:  # Ensure we have at least 4 cards
                    # Take first 4 cards (auction cards)
                    auction_hand = hand[:4]
                    
                    # Canonicalize the hand
                    canonical_hand, suit_mapping = self.canonicalizer.canonicalize_hand(auction_hand)
                    
                    # Get hand features
                    features = self.canonicalizer.get_hand_features(auction_hand)
                    
                    # Use the actual points from the example
                    actual_points = example.get('actual_points', 0.0)
                    if actual_points == 0.0:
                        # Estimate based on success and bid
                        if example.get('success', False):
                            actual_points = example.get('bid', 16) * 0.8  # Successful bid
                        else:
                            actual_points = example.get('bid', 16) * 0.3  # Failed bid
                    
                    # Create training example
                    training_example = {
                        'canonical_hand': canonical_hand,
                        'original_hand': auction_hand,
                        'features': features,
                        'actual_points': actual_points,
                        'player_id': example.get('position', 0),
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'features': torch.tensor([
                ...
                example['features']['longest_suit_count'] / 4.0,
    ...
    def hand_to_tensor(self, canonical_hand: Tuple[str, ...]) -> torch.Tensor:
        ...
        # Create a 4x13 tensor (4 cards, 13 possible ranks)
        tensor = torch.zeros(4, 13)
...
class PointPredictionModel(nn.Module):
    ...
    def forward(self, hand_tensor, features):
        # Process hand
        hand_flat = hand_tensor.view(hand_tensor.size(0), 4, -1)  # (batch, 4, 13)
...
class PointPredictionTrainer:
    ...
    def predict_points(self, hand: List[str]) -> float:
        """Predict expected points for a given 4-card hand"""
        self.model.eval()
        ...
        feature_tensor = torch.tensor([
            ...
            features['longest_suit_count'] / 4.0,
