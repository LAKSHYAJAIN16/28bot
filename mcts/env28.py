import copy
import random
from .constants import SUITS, RANKS, card_suit, card_rank, card_value, rank_index, suit_trump_strength
        ...
        if initial_trump is not None:
            ...
            while safety < 200:
                ...
                if proposal is not None and 16 <= proposal <= 28 and proposal >= min_allowed:
                    ...
                    if self.debug:
                        print(
                            f"Bid: Player {p} proposes {proposal} (min_allowed={min_allowed}) – suit_scores={dbg['suit_to_points'] if dbg else {}} chosen={dbg['chosen_suit'] if dbg else None} p30={dbg['p30_points'] if dbg else None} avg={dbg['avg_points'] if dbg else None}"
                        )
                else:
                    passed[p] = True
                    if self.debug:
                        print(f"Pass (locked): Player {p} (min_allowed={min_allowed}) – analysis={dbg}")
            ...
            if self.debug:
                print(
                    f"Auction winner: Player {self.bidder} with bid {self.bid_value}; chooses trump {self.trump_suit} – details={choose_dbg}"
                )
