...
class GameDataExtractor:
    ...
    def extract_game_data(self, log_file: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract bidder and bid - handle multiple formats
            bidder = None
            bid_value = None
            
            # Format 1: "Auction winner (bidder): Player X with bid Y"
            ...
            # Format 2: "Auction winner: Player X with bid Y"
            if not bidder_match:
                bidder_match = re.search(r'Auction winner: Player (\d+) with bid (\d+)', content)
                if bidder_match:
                    bidder = int(bidder_match.group(1))
                    bid_value = int(bidder_match.group(2))
            
            if bidder is None or bid_value is None:
                return None
            
            # Extract trump suit - handle multiple formats
            trump_suit = None
            
            # Format 1: "Bidder sets concealed trump suit: X"
            trump_match = re.search(r'Bidder sets concealed trump suit: (\w)', content)
            if trump_match:
                trump_suit = trump_match.group(1)
            
            # Format 2: "chooses trump X"
            if not trump_match:
                trump_match = re.search(r'chooses trump (\w)', content)
                if trump_match:
                    trump_suit = trump_match.group(1)
            ...
            # Extract bidding analysis for the bidder - handle multiple formats
            bidding_stats = {}
            
            # Format 1: Detailed bidding analysis with suit_stats
            ...
            # Format 2: Simpler format with analysis= and suit_scores=
            if not bidding_stats:
                # Look for analysis= patterns
                analysis_pattern = rf'Player {bidder}.*?analysis=\{{(.*?)\}}'
                analysis_match = re.search(analysis_pattern, content)
                if analysis_match:
                    analysis_text = analysis_match.group(1)
                    # Extract suit_to_points
                    suit_points_pattern = r"'(\w)': \[([\d, ]+)\]"
                    suit_points_matches = re.findall(suit_points_pattern, analysis_text)
                    for suit, pts_str in suit_points_matches:
                        pts = [int(x.strip()) for x in pts_str.split(',') if x.strip()]
                        if pts:
                            bidding_stats[suit] = {
                                'points': pts,
                                'avg': sum(pts) / len(pts),
                                'p30': sorted(pts)[int(len(pts) * 0.3)] if len(pts) > 0 else 0,
                                'std': np.std(pts) if len(pts) > 1 else 0
                            }
    ...
    def extract_all_games(self) -> List[Dict]:
        """Extract data from both training.log and individual game log files."""
        games_data = {}
        
        # First, extract from training.log
        try:
        ...
        # Then, extract from individual game log files
        import glob
        import os
        
        log_files = glob.glob(os.path.join(self.logs_dir, "*.log"))
        print(f"Found {len(log_files)} individual game log files")
        ...
        for log_file in log_files:
            try:
                ...
                if game_data:
                    ...
                    if game_id not in games_data:  # Don't overwrite training.log data
                        games_data[game_id] = game_data
            except Exception as e:
                print(f"Error processing {log_file}: {e}")
        
        # Only keep games that have complete data
        complete_games = [game for game in games_data.values() 
                         if game['team_a_points'] is not None and game['team_b_points'] is not None]
        
        # Remove the 0-point filter - these are valid games
        valid_games = complete_games
        
        print(f"Extracted complete data from {len(complete_games)} games total")
        print(f"Using all {len(valid_games)} games (including 0-point games)")
        
        # For games from training.log, estimate individual player points based on team scores and hand strength
        for game in valid_games:
            if 'player_points' not in game:  # Only for training.log games
